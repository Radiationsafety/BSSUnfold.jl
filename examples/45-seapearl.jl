...
> **Version note.** SeaPearl 0.4.x supports Julia 1.8–1.9 while BSSUnfold.jl
> requires Julia 1.10. Sections 1–2 always run (on Julia 1.10); section 3
> additionally requires a SeaPearl-capable environment and is skipped
> gracefully when SeaPearl is not loadable — see the notes there.
"""
...
* Training script: `seapearl_training/train_seapearl_bss.jl` (Julia 1.9
  side-environment; SeaPearl 0.4.5 + Flux).
...
> **Version note.** On plain Julia 1.10 SeaPearl is not installable
> (`julia = "1.8 - 1.9"` upstream); this cell then prints a hint instead of
> failing. In a SeaPearl-capable environment it runs a head-to-head
> comparison at `n_levels = 8` against the same `BasicHeuristic` settings.
"""
...
if rl_result.loaded
    ...
    To enable it: use a Julia 1.9 environment with
    `Pkg.add(name = "SeaPearl", version = "0.4.5")` and `Flux`, then run this
    notebook there, or retrain from scratch with
    `seapearl_training/train_seapearl_bss.jl`.
