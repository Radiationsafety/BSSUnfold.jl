# Tests of the port of dose_calculation.py, utils/interpolation.py and constants.py.
...
@testset "Dose calculation" begin
    ...
    # constant spectrum: values grow with the coefficients
    ks = cc["AP"]
    ...
    @test all(intpl["AP"] .>= 0)
    # out of range — fill_value = 0
    @test interpolate_coefficients(cc, [1e-12, 2000.0])["AP"] == [0.0, 0.0]

    # different coefficient sets are registered
...
@testset "Interpolation" begin
    ...
    @test all(isfinite.(v))
    @test all(v .>= 0)
    # outside the source range → fill_value = 0
    @test interpolate_spectrum(spec, E, [1e-10, 1e-3]; fill_value=0.0)[1] == 0.0

    # PCHIP preserves monotonicity on monotonic data
    ...
    # comparison with SciPy (benchmark: v == PchipInterpolator(...), binned at 4 points)
...
@testset "Detector (real RF)" begin
    ...
    # switching to a different coefficient set
    ...
    # effective readings from a real reference spectrum
    ...
    if isfile(csv)
        ...
        # In the CSV the grid is 61 bins, the detector is 60 bins: testing reading interpolation
        ...
        # the ported dose calculation matches Python-bssunfold (see benchmark values)
